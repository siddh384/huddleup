...
export async function createCourt(courtData: {
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Unauthorized: You can only create courts for your own venues",
      };
    }

    // Check if the sport is available in this venue
    const venueHasSport = venue.venueSports.some(
      (vs) => vs.sportId === courtData.sportId,
    );
    if (!venueHasSport) {
      return {
        success: false,
        error: "This sport is not available at this venue",
      };
    }
    ...
    if (startTime >= endTime) {
      return {
        success: false,
        error: "Operating hours start time must be before end time",
      };
    }
...
export async function getVenueCourts(venueId: string) {
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Unauthorized: You can only view courts for your own venues",
      };
    }
...
export async function getVenueSports(venueId: string) {
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Unauthorized: You can only view sports for your own venues",
      };
    }
...
export async function updateCourt(
  ...
  try {
    ...
    // Check if user owns the venue or is admin
    if (
      court.venue.ownerId !== userResult.user.id &&
      userResult.user.role !== "admin"
    ) {
      return {
        success: false,
        error: "Unauthorized: You can only update courts for your own venues",
      };
    }

    // If sport is being changed, check if it's available in the venue
    if (courtData.sportId) {
      const venueHasSport = court.venue.venueSports.some(
        (vs) => vs.sportId === courtData.sportId,
      );
      if (!venueHasSport) {
        return {
...
export async function deleteCourt(courtId: string) {
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Unauthorized: You can only delete courts for your own venues",
      };
    }
