...
import { venues, courts, sports, venueSports, reviews, bookings, user } from "@/db/schema";
import { db } from "@/db";
...
export async function createVenue(venueData: {
    ...
    try {
        ...
        if (userResult.user.role !== 'facility_owner' && userResult.user.role !== 'admin') {
            return { success: false, error: "Unauthorized: Facility owner access required" };
        }
...
export async function updateVenueStatus(venueId: string, status: 'approved' | 'rejected', rejectionReason?: string) {
    try {
        const userResult = await getCurrentUser();
...
export async function getOwnerDashboardStats() {
    try {
        ...
        const totalCourts = userVenues.reduce((sum, venue) => sum + venue.courts.length, 0);
        ...
        if (venueIds.length > 0) {
            ...
            totalRevenue = allBookings.reduce((sum, booking) => sum + parseFloat(booking.totalPrice), 0);
            ...
            const monthlyBookingsData = allBookings.filter(booking =>
                new Date(booking.bookingDate) >= currentMonth
            );

            monthlyBookings = monthlyBookingsData.length;
            monthlyRevenue = monthlyBookingsData.reduce((sum, booking) => sum + parseFloat(booking.totalPrice), 0);
        }
