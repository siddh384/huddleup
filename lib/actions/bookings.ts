...
export async function generateTimeSlots(courtId: string, date: string) {
  try {
    ...
    const [startHour, startMinute] = court.operatingHoursStart
      .split(":")
      .map(Number);
  ...
  try {
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      if (bookedHours.has(hour)) {
        return {
          success: false,
          error: `Time slot ${hour}:00 - ${hour + 1}:00 is already booked`,
        };
      }
    }
...
export async function getCourtBookings(courtId: string) {
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Unauthorized: You can only view bookings for your own courts",
      };
    }
